...
import { searchUsers, searchUserByEmail, followUser, unfollowUser, isFollowing as checkFollowing, getUserPublicPlaylists } from '@/services/socialService';
        ...
        <div style={{ padding:'0 20px 16px', flexShrink:0 }}>
          <div style={{ display:'flex', gap:8 }}>
            ...
            <button onClick={() => search(query)} style={{ padding:'11px 18px', borderRadius:12, background:'var(--pink)', border:'none', color:'#fff', fontWeight:700, fontSize:13, cursor:'pointer', fontFamily:'inherit' }}>
              {loading ? <Loader size={14} style={{ animation:'spin 0.8s linear infinite' }} /> : 'Search'}
            </button>
          </div>
        </div>
        ...
        <div style={{ flex:1, overflowY:'auto', padding:'0 20px' }}>
          ...
          {results.map(u => (
            <div key={u.uid} style={{ display:'flex', alignItems:'center', gap:12, padding:'12px 0', borderBottom:'1px solid rgba(255,255,255,0.05)' }}>
              <div style={{ width:44, height:44, borderRadius:'50%', overflow:'hidden', background:'linear-gradient(135deg,#FF6B9D,#E05587)', flexShrink:0, cursor:'pointer' }} onClick={() => onViewUser(u)}>
              ...
              <button onClick={() => handleFollow(u)} style={{ padding:'7px 14px', borderRadius:20, border:`1.5px solid ${following.has(u.uid) ? 'rgba(255,255,255,0.15)' : 'var(--pink)'}`, background: following.has(u.uid) ? 'rgba(255,255,255,0.05)' : 'rgba(255,107,157,0.1)', color: following.has(u.uid) ? 'var(--text-muted)' : 'var(--pink)', fontWeight:700, fontSize:12, cursor:'pointer', fontFamily:'inherit', display:'flex', alignItems:'center', gap:5 }}>
                {following.has(u.uid) ? <><UserCheck size={13} /> Following</> : <><UserPlus size={13} /> Follow</>}
              </button>
            </div>
          ))}
        </div>
      </div>
      <style>{`@keyframes sm-slide{from{transform:translateY(100%)}to{transform:translateY(0)}} @keyframes spin{to{transform:rotate(360deg)}}`}</style>
    </>
  );
}